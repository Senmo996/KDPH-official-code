...
@registry.register_runner("KDPHTrainer")
class KDPHTrainer(BaseTrainer):
    def __init__(self,
                cfg, 
                is_train=True, 
                logger=None, 
    ...
    def build_optimizer(self, cfg_optimizer, parameters=None):
        ...
        optimizer_loss = torch.optim.AdamW(params=self.model.hyp.parameters(), lr=lr_, weight_decay=weight_decay_)
        self.logger.info("Building optimizer!")
        return optimizer, optimizer_loss, lr_schedu

    def compute_loss(self, img_hash, txt_hash=None, label=None, index=None, epoch=0, times=0, global_step=0, **kwags):
